// Author: QHZY
// Create_Time: 2025/08/11 12:02:27
#include <bits/stdc++.h>
using namespace std;
...
vi find_occurrences(vi &text, vi &pattern) {
    vi cur = pattern;
    cur.eb(-1);
    cur.insert(cur.end(), all(text));
...
void work() {
    ...
    if (n == k) {
        vi aa = a;
        aa.insert(aa.end(), all(a));
        vi kmp_ans = find_occurrences(aa, b);
        YES(size(kmp_ans));
        return;
    }
    if (k % 2 == 0) {
    ...
    int counta = compute(a, n);
    int countb = compute(b, n);
    YES(counta == countb);
}
signed main() {
