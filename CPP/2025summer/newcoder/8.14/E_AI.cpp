// Author: QHZY
// Create_Time: 2025/08/15 02:19:41
#include <bits/stdc++.h>
using namespace std;
...
void work() {
    ...
    FOR(v1, 0, lim) {
        FOR(v2, v1 + 1, lim) {
            ...
            FOR(i, 1, n) {
                if (v1 >= a[i]) {
                    int from_v1 = (dp[i - 1][0] == INF) ? INF : dp[i - 1][0] + abs(a[i - 1] - a[i]);
                    int from_v2 = (dp[i - 1][1] == INF) ? INF : dp[i - 1][1] + abs(v1 - v2 + a[i - 1] - a[i]);
                    dp[i][0] = min(from_v1, from_v2);
                } else {
                    dp[i][0] = INF;
                }
                if (v2 >= a[i]) {
                    int from_v1 = (dp[i - 1][0] == INF) ? INF : dp[i - 1][0] + abs(v2 - v1 + a[i - 1] - a[i]);
                    int from_v2 = (dp[i - 1][1] == INF) ? INF : dp[i - 1][1] + abs(a[i - 1] - a[i]);
                    dp[i][1] = min(from_v1, from_v2);
                } else {
                    dp[i][1] = INF;
                }
            }
            if (dp[n - 1][0] != INF) {
                ans = min(ans, (dp[n - 1][0] + abs(v1 - a[n - 1])) / 2);
            }
            if (dp[n - 1][1] != INF) {
                ans = min(ans, (dp[n - 1][1] + abs(v2 - a[n - 1])) / 2);
            }
        }
    }
    cout << ans << endl;
