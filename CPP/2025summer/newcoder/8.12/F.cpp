// Author: QHZY
// Create_Time: 2025/08/12 12:18:27
#include <bits/stdc++.h>
using namespace std;
...
void work() {
    ...
    int ans = max(abs(sx1 - tx1), abs(sy1 - ty1)) + max(abs(sx2 - tx2), abs(sy2 - ty2));
    ans = min(ans, max(abs(sx1 - tx2), abs(sy1 - ty2)) + max(abs(sx2 - tx1), abs(sy2 - ty1)));
    cout << ans << endl;
}
signed main() {
